...
pub type BinaryLCTree<H> =
    LCMerkleTree<<H as Hasher>::Domain, <H as Hasher>::Function, typenum::U2>;
...
impl<H: Hasher, G: Hasher> TemporaryAux<H, G> {
    ...
    // 'delete' will discard all persisted merkle and layer data that
...
impl<H: Hasher, G: Hasher> TemporaryAuxCache<H, G> {
    pub fn new(t_aux: &TemporaryAux<H, G>, replica_path: PathBuf) -> Result<Self> {
        ...
        let tree_r_last: QuadLCTree<H> = create_lcmerkle_tree::<H, typenum::U4>(
